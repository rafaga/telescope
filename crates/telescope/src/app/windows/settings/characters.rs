...
impl TelescopeApp {
    pub(super) fn show_characters_page(&mut self, ui: &mut egui::Ui) {
        ...
        ui.label(
            "These are used to emit notifications when something it is close to your location.",
        );
        ui.horizontal(|ui| {
            ...
            if ui
                .add_enabled(button_state, egui::Button::new("✖ Remove"))
                .clicked()
            {
                let mut index = 0;
                let mut vec_id = vec![];
                        ...
                        if active_char == char.id {
                            if let Some(sender) = &self.char_msg {
                                ...
                                let _result = runtime.block_on(async {
                                    sender.send(CharacterSync::Remove(char.id as usize)).await
                                });
                            }
                            vec_id.push(char.id);
                            break;
                            ...
                            ui.group(|ui| {
                                ui.push_id(self.esi.characters[index].id, |ui| {
                                    let inner = ui.horizontal_centered(|ui| {
                                        ...
                                        if let Some(player_photo) =
                                            &self.esi.characters[index].photo
                                        {
                                            ui.add(
                                                eframe::egui::Image::new(player_photo.as_str())
                    ...
                    body.row(200.00, |mut row| {
                        row.col(|ui| {
                            ui.add_sized(
                                ui.available_size(),
                                egui::Label::new(
                                    "⚠ There is no characters currently linked in Telescope.",
                                ),
                            );
                        });
                    });
